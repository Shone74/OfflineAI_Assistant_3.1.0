...
class PathValidator:
    ...
    read_roots
        ...
        path pass basic sanitisation.
    write_roots
        Allowed root directories for writing.  When empty and *enabled* is
        True, write operations are denied (Unconfigured state).
    follow_symlinks
    ...
    def _validate(
        ...
        # Convert relative paths to absolute for consistent resolution
        ...
        # If roots are configured, enforce containment
        if roots:
...
def configure_default_validator(
    ...
    changes filesystem settings in the Settings dialog.
    """
...
def configure_default_validator_from_config(
    ...
    return configure_default_validator(
        read_roots=list(fs_config.read_roots),
        write_roots=list(fs_config.write_roots),
